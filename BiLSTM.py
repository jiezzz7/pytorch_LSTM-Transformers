...
def train_model(model, train_loader, val_loader, test_loader, criterion, optimizer, num_epochs=20):
    ...
    if best_model:
        ...
        model_dir = current_dir / 'BiLSTM_model'
...
def plot_predictions(data_handler, model, images_dir):
    ...
    plt.figure(figsize=(20, 5))
    plt.title('BiLSTM Close Price Validation', fontsize=20, pad=20)
    ...
    if not trainPredictPlot.empty:
        ...
        plt.text(train_start, max_close, 'Train', fontsize=15, verticalalignment='top')  # 参考资料 [^1]

    if not valPredictPlot.empty:
        val_start = valPredictPlot.index[0]
        plt.axvline(x=val_start, color='black', linestyle='--')
        plt.text(val_start, max_close, 'Validation', fontsize=15, verticalalignment='top')  # 参考资料 [^1]

    if not testPredictPlot.empty:
        test_start = testPredictPlot.index[0]
        plt.axvline(x=test_start, color='black', linestyle='--')
        plt.text(test_start, max_close, 'Test', fontsize=15, verticalalignment='top')  # 参考资料 [^1]

    plt.ylabel('Close Price USD ($)', fontsize=18)
    plt.xlabel('Date', fontsize=16)  # 添加 X 轴标签
    plt.legend(loc='upper right')
    plt.tight_layout()  # 确保坐标轴和标题正确显示
...
def main():
    ...
    images_dir = current_dir / 'BiLSTM_image'
    ...
    # 模型初始化
    model = BiLSTM(input_size=data_handler.X_train.shape[2], hidden_size=64, num_layers=3, output_size=1).to(device)
    criterion = nn.MSELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=0.0001)

    # 输出模型概要
    summary(model, (32, 60, data_handler.X_train.shape[2]), device=device)
    ...
    # 加载最佳模型
    model.load_state_dict(torch.load(current_dir / 'BiLSTM_model' / 'model_best.pth'))
