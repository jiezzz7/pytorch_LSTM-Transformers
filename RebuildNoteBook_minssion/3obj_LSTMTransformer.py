...
class MultiOutputLSTMTransformer(nn.Module):
    def __init__(self,
                 input_dim,
                 hidden_dim,
                 lstm_layers,
        ...
        super(MultiOutputLSTMTransformer, self).__init__()
        # 共享的LSTM和Transformer层
        ...
        # 各个任务的输出层
        self.fc_vp = nn.Linear(hidden_dim, 1)
        self.fc_vs = nn.Linear(hidden_dim, 1)
        self.fc_ratio = nn.Linear(hidden_dim, 1)
        self.fc_mile = nn.Linear(hidden_dim, 1)
...
def main():
    ...
    # 创建模型
    model = MultiOutputLSTMTransformer(**params).to(device)
